...
Ignite UI for Angular で、**並べ替え**も列ごとのレベルで有効化されます。つまり、`igx-grid` に並べ替え可能な列および並べ替え可能ではない列の両方を含むことが可能です。
...
`sortable` 入力によって実行できます。グリッド フィルターと同じように、`sortingIgnoreCase` プロパティを設定すると、大文字と小文字を区別する並べ替えを実行できます。
...
> [!NOTE]
> If values of type `string` are used by column of dataType `Date`, the grid won't parse it to `Date` objects and using of igxGrid `sorting` won't wont work as expected. If you want to use `string` objects, additional logic should be implemented on application level, in order to parse the values to `Date` object.
