...
### Dock Manager のデータ分析
選択したデータに基づいて `Chart Types ビュー`を有効にするには、`セル範囲の選択`または`列の選択`を実行します。このビューは、[Dock Manager](../../dock-manager-overview.md) の右ペインの一部です。以下のオプションを選択できます。
...
> [!NOTE]
> [Dock Manager Web コンポーネント](../../dock-manager-overview.md)は、ペインでアプリケーションのレイアウトを管理する方法を提供します。エンド ユーザーはペインをピン固定、サイズ変更、移動、非表示にすることでカスタマイズできます。データを選択した後、いくつかのチャートを作成し、利用可能な領域にドラッグしてピン固定します。
