...
列選択機能は、[GridSelectionMode]({environment:angularApiUrl}/index.html#gridselectionmode) 値を受け取る [`columnSelection`]({environment:angularApiUrl}/classes/@@igTypeDoc.html#columnselection) 入力によって有効にすることができます。
...
> [!NOTE]
> キーボードの組み合わせは、グリッドの [`columnSelection`]({environment:angularApiUrl}/classes/@@igTypeDoc.html#columnselection) 入力が `multiple` に設定されている場合にのみ使用できます。
