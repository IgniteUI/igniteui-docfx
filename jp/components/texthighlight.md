...
### 複数コンテナで検索
igxTextHighlight は、複数のコンテナ内を検索し、アクティブ ハイライトを共有できます。すべて個別のコンテナを持つ複数の TextHighlight ディレクティブで同じグループ値を持つことにより実行できます。サンプルの設定は、前のサンプルの検索ボックスを再利用しますが、今回は 2 段落追加します。両方ともコンテナにありますが 2 つ目は行に異なる値があります。
