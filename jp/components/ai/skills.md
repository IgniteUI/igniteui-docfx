...
Claude Code は、`.claude/skills/` (プロジェクト レベル) または `~/.claude/skills/` (グローバル) に配置されたスキルを検出します。各スキルは `SKILL.md` ファイルを含む独自のサブディレクトリとして存在する必要があります。

1. プロジェクト ルートに `.claude/skills/` ディレクトリを作成します。
2. `igniteui-angular/skills/` からスキル ディレクトリを `.claude/skills/` にコピーします。
...
3. Claude Code はすべてのセッション開始時に関連するスキルを自動的に検出して読み込みます。

> **注:** スキルのコンテンツを `CLAUDE.md` に直接貼り付けないでください。Claude Code に認識させるには、スキルを `.claude/skills/` (またはグローバルに利用できるようにするには `~/.claude/skills/`) の下に専用のサブディレクトリとして配置する必要があります。
