...
_description: Set custom column order of the Ignite UI for Angular table by using the column drag feature of the Angular UI grid with the usage of mouse/touch gestures, or by using the Column Moving API.
...
**열 이동** 기능은 열 단위 수준에서 사용할 수 있으며 [**igx-hierarchical-grid**]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html)에 이동 가능한 열 및 이동 불가한 열을 혼합하여 사용할 수 있습니다. 이것은 [`igx-column`]({environment:angularApiUrl}/classes/igxcolumncomponent.html)의 [`movable`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#movable) 입력을 통해 제어할 수 있습니다.
...
열의 드래그 앤드 드롭 조작을 설정하기 위해 열 이동과 관련된 복수의 이벤트가 제공됩니다. [`columnMovingStart`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#columnMovingStart), [`columnMoving`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#columnMoving) 및 [`columnMovingEnd`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#columnMovingEnd)가 있습니다. 
[`igx-hierarchical-grid`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html)의 [`columnMovingEnd`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#columnMovingEnd) 이벤트를 처리하고 열을 새로운 위치로 드롭할 때 일부 사용자 논리를 구현할 수 있습니다. 예를 들면, Change On Year(%) 열 뒤에 카테고리 드롭을 취소할 수 있습니다.
