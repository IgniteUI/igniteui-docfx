...
### Hierarchical Grid Excel 스타일 필터링

Ignite UI for Angular의 Hierarchical Grid 컴포넌트는 Excel과 유사한 필터 UI를 제공합니다.
...
Excel 스타일 필터링을 켜려면 2개의 입력을 설정해야 합니다. [`allowFiltering`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#allowfiltering)을 `true`로 설정하고 [`filterMode`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#filtermode)를 `excelStyleFilter`로 설정해야 합니다.
