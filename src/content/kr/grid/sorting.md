...
이것은 [`sortable`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#sortable) 입력을 통해 실행됩니다. Grid 정렬을 사용하면 [`sortingIgnoreCase`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#sortingignorecase) 속성을 설정하여 대소문자 구분을 실행할 수도 있습니다:
...
Grid [`sort`]({environment:angularApiUrl}/classes/igxgridcomponent.html#sort) 메소드를 사용하여 Grid API를 통해 모든 열 또는 열 조합을 정렬할 수 있습니다:
...
> [!NOTE]
> 정렬 조작은 Grid의 기본 데이터 소스를 변경하지 **않습니다**.

#### 초기 정렬 상태

Grid의 [`sortingExpressions`]({environment:angularApiUrl}/classes/igxgridcomponent.html#sortingexpressions) 속성에 정렬식 배열을 전달하여 Grid의 초기 정렬 상태를 설정할 수 있습니다.
...
> [!NOTE]
> `string` 유형의 값이 [`dataType`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#datatype) `Date` 열에서 사용되는 경우, Grid는 `Date` 객체로 구문 분석하지 않으므로 Grid를 사용하면 `sorting`이 예상대로 작동하지 않습니다. `string` 객체를 사용하려면 값을 `Date` 객체로 구문 분석하기 위해 애플리케이션 수준에서 추가 논리를 구현해야 합니다.

<div class="divider--half"></div>


#### 원격 정렬
[`dataPreLoad`]({environment:angularApiUrl}/classes/igxgridcomponent.html#datapreload) 및 [`sortingDone`]({environment:angularApiUrl}/classes/igxgridcomponent.html#sortingDone) 출력에 서브스크라이브하여 Grid의 원격 정렬을 제공할 수 있습니다. 사용 방법에 대한 자세한 정보는 `Grid 가상화 및 성능` [문서](virtualization.md#remote-sortingfiltering-virtualization)를 참조하십시오.
