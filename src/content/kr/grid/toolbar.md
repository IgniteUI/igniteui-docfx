...
단순히 도구 모음 기능을 켜거나 끄는 것 외에도 내보내기 처리는 사용자가 내보내기 처리를 초기화할 때 생성되는 [`toolbarExporting`]({environment:angularApiUrl}/classes/igxgridcomponent.html#toolbarExporting) 이벤트에서 상세 설정할 수 있습니다. 이 이벤트는 내보내기와 내보내기 옵션을 모두 보여줍니다.
...
다음 코드 조각은 [`toolbarExporting`]({environment:angularApiUrl}/classes/igxgridcomponent.html#toolbarExporting) 이벤트에 서브스크라이브하는 방법과 기본 내보내기 처리를 취소하는 방법을 보여줍니다.

```html
<igx-grid ... (toolbarExporting)="toolbarExportingHandler($event)"></igx-grid>
```
...
> `grid` - 도구 모음이 포함된 [`IgxGridComponent`]({environment:angularApiUrl}/classes/igxgridcomponent.html)에 대한 참조 및
...
[`IgxGridComponent`]({environment:angularApiUrl}/classes/igxgridcomponent.html) 속성:
