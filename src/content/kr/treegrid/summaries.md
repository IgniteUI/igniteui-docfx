...
**Ignite UI for Angular에서 Tree Grid summaries**을 열 단위 수준으로 활성화하고 필요한 열에 대해서만 활성화할 수 있습니다. Tree Grid 요약은 열의 데이터 유형에 따라 사전 정의된 기본 요약 세트를 제공하므로 시간을 절약할 수 있습니다.
...
**[`hasSummary`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#hassummary) 속성을 `true`로 설정하면 Tree Grid 요약**이 열 단위로 활성화됩니다. 또한, 각 열의 요약은 열 데이터 형식에 따라 해결되는 것에 유의하십시오. `igx-tree-grid`에서 기본 열 데이터 유형은 `string`이므로 `number` 또는 `date` 별 요약을 원하는 경우, [`dataType`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#datatype) 속성을 `number` 또는 `date`로 설정해야 합니다.
...
특정 열 또는 열 목록에서 요약을 활성화/비활성화하는 또 다른 방법은 **igx-tree-grid**의 공개 메소드 [`enableSummaries`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#enablesummaries)/[`disableSummaries`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#disablesummaries)를 사용하는 것입니다.
...
> [!NOTE]
> 요약 행 높이를 올바르게 계산하기 위해 Tree Grid의 [`operate`]({environment:angularApiUrl}/classes/igxsummaryoperand.html#operate) 메소드에서 데이터가 비어있는 경우에도 항상 적절한 길이의 [`IgxSummaryResult`]({environment:angularApiUrl}/interfaces/igxsummaryresult.html) 배열을 반환해야 합니다.
...
#### Custom summaries, which access all Tree Grid data
...
Tree Grid는 루트 노드와 각 중첩된 하위 노드 수준에 대해 별도의 요약을 지원합니다. 표시된 요약은 [`summaryCalculationMode`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#summarycalculationmode) 속성을 사용하여 구성할 수 있습니다. 하위 수준 요약은 [`summaryPosition`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#summaryposition) 속성을 사용하여 하위 노드 전후에 표시할 수 있습니다.

[`summaryCalculationMode`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#summarycalculationmode) 속성의 사용 가능한 값은 다음과 같습니다:
...
[`summaryPosition`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#summaryposition) 속성의 사용 가능한 값은 다음과 같습니다:
 - top - 요약 행은 하위 행의 목록 앞에 나타납니다.
 - bottom - 요약 행은 하위 행의 목록 뒤에 나타납니다. 이것이 기본값입니다.

> [!NOTE]
> [`summaryPosition`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#summaryposition) 속성은 하위 수준 요약에만 적용됩니다. 루트 수준 요약은 Tree Grid의 아래에 항상 고정되어 표시됩니다.
...
* [IgxTreeGridSummaries 스타일]({environment:sassApiUrl}/themes#function-grid-summary-theme)
