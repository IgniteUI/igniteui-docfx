...
The [`IgxLinearGauge`](/components/lineargauge.html) requires the following modules:
...
The scale is a visual element that highlights the full range of values in the linear gauge. You can customize the appearance and the shape of the scale. It can also be inverted (using [`isScaleInverted`](/components/lineargauge.html) property) and all labels will be rendered from right-to-left instead of left-to-right.
