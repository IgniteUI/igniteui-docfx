...
The bullet graph supports one scale, one set of tick marks and one set of labels. The bullet graph component also has built-in support for animated transitions. This animation is easily customizable by setting the [`transitionDuration`](/components/bulletgraph.html) property.
...
The [`IgxBulletGraphComponent`](/components/bulletgraph.html) requires the following modules:
...
The scale is visual element that highlights the full range of values in the gauge. You can customize appearance and shape of the scale. The scale can also be inverted (using [`isScaleInverted`](/components/bulletgraph.html) property) and all labels will be rendered from right-to-left instead of left-to-right.
