...
### Hierarchical Grid Column Resizing
...
#### Hierarchical Grid Column Resizing Demo
...
Pinned columns can also be resized. However, resizing is limited so that the overall width of the pinned columns container cannot become larger than 80% of the overall Hierarchical Grid width.
Again, if you try to autosize a pinned column and the new width will cause the pinned columns container to exceed those 80% of the overall Hierarchical Grid width, autosizing will be discarded. We just want to make sure that the unpinned columns are always visible and available to the user!
