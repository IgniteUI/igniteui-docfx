...
The **autoGenerate** property tells the **igx-grid** to autogenerate columns based on the data source fields. It will also try to deduce the appropriate data type for the column if possible. Otherwise, the developer needs to explicitly define the columns and the mapping to the data source fields.
...
The column also accepts one last template that will be used when a cell is in edit mode. As with the other column templates, the provided context object is again the cell value and the cell object itself. Of course in order to have your template show up make sure to set the `editable` property of the column to true.
...
Each of the column templates can be changed programmatically at any point through the column object itself. For example in the code below, we have declared two templates for our user data. In out Typescript code we'll get references to the templates itself and then based on some condition we will render the appropriate for the column in our application.
...
The code above will make the **ProductName** column sortable, filterable, and editable and will instantiate the corresponding features UI (like inputs for editing, etc.).

### Data binding

Before going any further with the grid we want to change the grid to bind to remote data service, which is a common scenario in real production applications. A good practice is to separate all data fetching related logic in a separate data service, so we are going to create a service which will handle the fetching of data from the server.
...
We're importing the `Injectable` decorator which is an [essential ingredient](https://angular.io/guide/dependency-injection) in every Angular service definition. The `HttpClient` will provide us with the functionality to communicate with backend services. It returns an `Observable` of some result on which we will subscribe in our grid component.
...
After implementing the service we will inject it in our components constructor and use it to retrieve the data. The `ngOnInit` lifecycle hook is a good place to dispatch the initial request.
...
The grid supports both horizontal and vertical virtualization. In order to activate it make sure to set either the `height` or `width` property of the grid.
...
The grid component exposes and `addRow` method which will add the provided data to the data source itself.
...
The only required parameters are the column field and the filtering term. Both the condition and the case sensitivity will be infered from the column properties if not provided. In the case of multiple filtering, the method accepts an array of filtering expressions.
...
It is possible to set the initial sorting state of the grid by an array of expressions to the `sortingExpressions` property of the grid.
...
|`onPagingDone`|Emitted when paging is performed. Returns an object consisiting of the previous and the new page.|
...
|`deleteRow(rowIndex: number)`|Removes the row object and the data recrod corresponding to it from the data source.|
...
|`paginate(page: number)`|Goes to the specified page if paging is enabled. Page indexes are 0 based.|
|`markForCheck()`|Manually triggers a changed detection cycle of the grid and its children.|
...
|`inlineEditorTemplate`|TemplateRef|Yes|Yes|Get/Set a reference to a template which will be applied as a cell enters in edit mode.|
