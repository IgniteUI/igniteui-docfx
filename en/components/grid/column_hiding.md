...
The Ignite UI for Angular Grid provides an [`IgxColumnHidingComponent`]({environment:angularApiUrl}/classes/igxcolumnhidingcomponent.html), which allows users to hide and show columns directly through the **UI**. The Grid has a built-in column hiding UI, which can be used through the Grid's toolbar. In addition, developers can always define the column hiding UI as a separate component and place it anywhere they want on the page.
