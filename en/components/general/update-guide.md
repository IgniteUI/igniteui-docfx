...
* Template
    * The new structure defines tab items each wrapping a header and a content. The headers may contain a label and/or an icon as well as any other custom content.
    * For header styling purposes we introduced four new directives, namely `IgxBottomNavHeaderLabel`, `IgxBottomNavHeaderIcon`, `IgxTabHeaderLabel` and `IgxTabHeaderIcon`.
    * Since the header component now allows adding any content, the `IgxTab` directive, which was previously used to retemplate the tab's header, was removed because it is no longer necessary.
    ...
    ```
* The **API** of the components was changed during the refactoring.
    ...
    * The following properties were changed:
        * The tab item's `isSelected` was renamed to `selected`.
    ...
    * **IgxBottomNav** specific members
        * The `selectedTab` property was renamed to `selectedItem`.
        * The `itemStyle` property was removed.
