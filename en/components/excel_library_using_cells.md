...
You can access a [`WorksheetCell`](/components/excel_library_using_cells.html) object or a [`WorksheetRegion`](/components/excel_library_using_cells.html) object by calling the [`worksheet`](/components/excel_library_using_cells.html) object’s `getCell` or `getRegion` methods, respectively. Both methods accept a string parameter that references a cell. Getting a reference to a cell is useful when applying formats or working with formulas and cell contents.
...
The Infragistics Angular Excel Library supports the referencing of cells and regions by name through the `getCell` and `getRegion` methods of the [`worksheet`](/components/excel_library_using_cells.html) object. You refer to the cell or region using the [`NamedReference`](/components/excel_library_using_cells.html) instance that refers to that cell or region.
...
The Infragistics Angular Excel Library allows you to add Microsoft Excel formulas to a cell or group of cells in a worksheet. You can do this using the [`WorksheetCell`](/components/excel_library_using_cells.html) object’s [`applyFormula`](/components/excel_library_using_cells.html) method or by instantiating a [`formula`](/components/excel_library_using_cells.html) object and applying it to a cell. Regardless of the manner in which you apply a formula to a cell, you can access the [`formula`](/components/excel_library_using_cells.html) object using the [`WorksheetCell`](/components/excel_library_using_cells.html) object’s [`formula`](/components/excel_library_using_cells.html) property. If you need the value, use the cell’s [`value`](/components/excel_library_using_cells.html) property.
...
The Infragistics Angular Excel Library allows you to customize the look and behavior of a cell. You can customize a cell by setting properties exposed by the `cellFormat` property of the [`WorksheetCell`](/components/excel_library_using_cells.html), [`WorksheetRow`](/components/excel_library_using_cells.html), [`WorksheetColumn`](/components/excel_library_using_cells.html), or [`WorksheetMergedCellsRegion`](/components/excel_library_using_cells.html) objects.
...
You can create all possible fill types using static properties and methods on the [`CellFill`](/components/excel_library_using_cells.html) class. They are as follows:
...
-   `createSolidFill` - Returns a [`CellFillPattern`](/components/excel_library_using_cells.html) instance which has a pattern style of `Solid` and a background color set to the `Color` or [`WorkbookColorInfo`](/components/excel_library_using_cells.html) specified in the method.

-   `createPatternFill` - Returns a [`CellFillPattern`](/components/excel_library_using_cells.html) instance which has the specified pattern style and the `Color` or [`WorkbookColorInfo`](/components/excel_library_using_cells.html) values, specified for the background and pattern colors.

-   `createLinearGradientFill` - Returns a [`CellFillLinearGradient`](/components/excel_library_using_cells.html) instance with the specified angle and gradient stops.

-   `createRectangularGradientFill` - Returns a [`CellFillRectangularGradient`](/components/excel_library_using_cells.html) instance with the specified left, top, right, and bottom of the inner rectangle and gradient stops. If the inner rectangle values are not specified, the center of the cell is used as the inner rectangle.

The derived types, representing the various fills which can be created, are as follows:

-   [`CellFillPattern`](/components/excel_library_using_cells.html) - A pattern that represents a cell fill of no color, a solid color, or a pattern fill for a cell. It has background color info and a pattern color info which correspond directly to the color sections in the Fill tab of the Format Cells dialog of Excel.

-   [`CellFillLinearGradient`](/components/excel_library_using_cells.html) - Represents a linear gradient fill. It has an angle, which is degrees clockwise of the left to right linear gradient, and a gradients stops collection which describes two or more color transitions along the length of the gradient.

-   [`CellFillRectangularGradient`](/components/excel_library_using_cells.html) - Represents a rectangular gradient fill. It has top, left, right, and bottom values, which describe, in relative coordinates, the inner rectangle from which the gradient starts and goes out to the cell edges. It also has a gradient stops collection which describes two or more color transitions along the path from the inner rectangle to the cell edges.

The following code snippet demonstrates how to create a solid fill in a [`WorksheetCell`](/components/excel_library_using_cells.html):
...
Colors are defined by the [`WorkbookColorInfo`](/components/excel_library_using_cells.html) class, which is a sealed immutable class. The class has a static `automatic` property, which returns the automatic color, and there are various constructors which allow you to create a [`WorkbookColorInfo`](/components/excel_library_using_cells.html) instance with a color or a theme value and an optional tint.

The `getResolvedColor` method on [`WorkbookColorInfo`](/components/excel_library_using_cells.html) allows you to determine what color will actually be seen by the user when they open the file in Excel.

If the [`WorkbookColorInfo`](/components/excel_library_using_cells.html) represents a theme color, you must pass in a Workbook instance to the method so it can get the theme color’s RGB value from the workbook.
...
You can set a host of different formats on a [`WorksheetCell`](/components/excel_library_using_cells.html) by using the [`cellFormat`](/components/excel_library_using_cells.html) object returned by the `cellFormat` property of that cell. This [`cellFormat`](/components/excel_library_using_cells.html) object enables you to style many different aspects of the cell such as borders, font, fill, alignments, and whether or not the cell should shrink to fit or be locked.

You can also access the built-in styles to Microsoft Excel 2007 using the `styles` collection of the [`Workbook`](/components/excel_library_using_cells.html) object. The full list of styles in Excel can be found in the Cell Styles gallery of the Home tab of Microsoft Excel 2007.
...
With this feature, a `style` property has been added to the `CellFormat` object. This is a reference to a [`WorkbookStyle`](/components/excel_library_using_cells.html) instance, representing the parent style of the format. For formats of a style, this property will always be null, because styles cannot have a parent style. For row, column, and cell formats, the `style` property always returns the `normalStyle` by default.
...
When you merge cells, each cell in the region will have the same value and cell format. The merged cells will also be associated with the same [`WorksheetMergedCellsRegion`](/components/excel_library_using_cells.html) object, accessible from their `associatedMergedCellsRegion` property. The resultant [`WorksheetMergedCellsRegion`](/components/excel_library_using_cells.html) object will also have the same value and cell format as the cells.
...
In order to create a merged cell region, you must add a range of cells to the [`Worksheet`](/components/excel_library_using_cells.html) object’s `mergedCellsRegions` collection. This collection exposes an `Add` method that takes four integer parameters. The four parameters determine the index of the starting row and column (top-left most cell) and the index of the ending row and column (bottom-right most cell).
