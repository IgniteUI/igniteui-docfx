...
The [igx-autocomplete]({environment:angularApiUrl}/classes/igxautocompletedirective.html) suggestions will show as long as you start typing in the text input. By default the first item is always selected, so that it can easily selected using the keyboard with Enter. The other way is to click an item from the list and the input value will be automatically updated and the drop down will be automatically closed, unless the onItemSelected event is handled and canceled. 
The igxAutocomplete directive allows you to navigate through items and in the same time to have focus always in the input. The latter will allow you to continue typing in the input. You can use ESC key to close the drop down list.

The [igx-autocomplete]({environment:angularApiUrl}/classes/igxautocompletedirective.html) uses the [ig-drop-down]({environment:angularApiUrl}/classes/igxdropdowncomponent.html) as a provider for the available options. This means that the available capabilities of the drop down component can be used - grouping, templates, disabling of items and groups.
</p>
<div class="divider"></div>

### Demo
The following sample demonstrates simple use of igx-autocomplete directive, applied on a igx-input, and provided with igx-drop-down with defined filtering pipe.
...
The following sample defines [`igxAutocompleteDisabled`]({environment:angularApiUrl}/classes/igxautocompletedirective.html#disabled), which allows to dynamically enable and disable the autocomplete drop down:
...
The following example defines positioning of the drop down to be always above the input, where the directive is applied. It also disables opening and closing animations. For the purpose `ConnectedPositioningStrategy` is used:
...
The following sample demonstrates autocomplete that allows picking a movie, where movies are grouped by genres. The drop down is bound to hierarchical data and is placed inside a Reactive Form:
...
> Note: Follow the [Template Driven Forms Integration](input_group.md) topic to see the same example and igxAutocomplete placed inside a Template Driven Form.
<div class="divider--half"></div>

### Autocomplete bound to remote data
The following sample demonstrates igxAutocomplete with drop down bound to a remote service. For the purpose of the sample there is a delay in order to apply a custom template, that will show instead of the drop down, when data is loading:
