...
The Infragistics Angular Excel Engine's [`Worksheet`](/components/excel_library_using_worksheets.html) is where your data is kept. You can input data by working with the Worksheet's rows and cells and setting their corresponding values. The [`Worksheet`](/components/excel_library_using_worksheets.html) allows you to filter, sort, and customize the formats of the cells, as shown below.
...
You can also use the [`Worksheet`](/components/excel_library_using_worksheets.html) object's `protect` method to protect a worksheet against structural changes.

When protection is set, you can set the `CellFormat` object's `locked` property on individual cells, rows, merged cell regions, or columns to override the worksheet object's protection on those objects. For example, if you need all cells of a worksheet to be read-only except for the cells of one column, you can protect the worksheet and then set the `CellFormat` object's `locked` property to `false` on a specific [`WorksheetColumn`](/components/excel_library_using_worksheets.html) object. This will allow your users to edit cells within the column while disabling editing of the other cells in the worksheet.
...
Filtering is done by setting a filter condition on a worksheet's [`WorksheetFilterSettings`](/components/excel_library_using_worksheets.html) which can be retrieved from the [`Worksheet`](/components/excel_library_using_worksheets.html) object's `filterSettings` property. Filter conditions are only reapplied when they're added, removed, modified, or when the `reapplyFilters` method is called on the worksheet. They are not constantly evaluated as data within the region changes.

You can specify the region to apply the filter by using the `setRegion` method on the [`WorksheetFilterSettings`](/components/excel_library_using_worksheets.html) object.

Below is a list of methods and their descriptions that you can use to add a filter to a worksheet:

| Method                                                                                                                              |                                                                                                    Description                                                                                                   |
| ----------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| [`applyAverageFilter`](/components/excel_library_using_worksheets.html)                     |                                            Represents a filter which can filter data based on whether the data is below or above the average of the entire data range.                                           |
| [`applyDatePeriodFilter`](/components/excel_library_using_worksheets.html)               |                                                                  Represents a filter which can filter dates in a Month, or quarter of any year.                                                                  |
| [`applyFillFilter`](/components/excel_library_using_worksheets.html)                           | Represents a filter which will filter cells based on their background fills. This filter specifies a single CellFill. Cells of with this fill will be visible in the data range. All other cells will be hidden. |
...
| [`applyFontColorFilter`](/components/excel_library_using_worksheets.html)                 |    Represents a filter which will filter cells based on their font colors. This filter specifies a single color. Cells with this color font will be visible in the data range. All other cells will be hidden.   |
| [`applyIconFilter`](/components/excel_library_using_worksheets.html)                           |                                                              Represents a filter which can filter cells based on their conditional formatting icon.                                                              |
| [`applyRelativeDateRangeFilter`](/components/excel_library_using_worksheets.html) |                                                    Represents a filter which can filter date cells based on dates relative to the when the filter was applied.                                                   |
| [`applyTopOrBottomFilter`](/components/excel_library_using_worksheets.html)             |                                                         Represents a filter which can filter in cells in the upper or lower portion of the sorted values.                                                        |
| [`applyYearToDateFilter`](/components/excel_library_using_worksheets.html)               |                              Represents a filter which can filter in date cells if the dates occur between the start of the current year and the time when the filter is evaluated.                              |
| [`applyCustomFilter`](/components/excel_library_using_worksheets.html)                       |                  Represents a filter which can filter data based on one or two custom conditions. These two filter conditions can be combined with a logical "and" or a logical "or" operation.                  |
...
In order to enable pane freezing, you need to set the `panesAreFrozen` property of the [`Worksheet`](/components/excel_library_using_worksheets.html) object's `displayOptions` to `true`. You can then specify the rows or columns to freeze by using the `frozenRows` and `frozenColumns` properties of the display options `frozenPaneSettings`, respectively.
...
You can change the zoom level for each worksheet independently using the `magnificationInNormalView` property on the [`Worksheet`](/components/excel_library_using_worksheets.html) object's `displayOptions`. This property takes a value between 10 and 400 and represents the percentage of zoom that you wish to apply.
...
This is done by specifying a region and sort type to the [`Worksheet`](/components/excel_library_using_worksheets.html) object's [`WorksheetSortSettings`](/components/excel_library_using_worksheets.html) that can be retrieved using the `sortSettings` property of the sheet.
...
You can protect a worksheet by calling the `protect` method on the [`Worksheet`](/components/excel_library_using_worksheets.html) object. This method exposes many nullable `bool` parameters that allow you to restrict or allow the following user operations:
...
You can remove worksheet protection by calling the `unprotect` method on the [`Worksheet`](/components/excel_library_using_worksheets.html) object.
...
You can configure the conditional formatting of a [`Worksheet`](/components/excel_library_using_worksheets.html) object by using the many "Add" methods exposed on the `conditionalFormats` collection of that worksheet. The first parameter of these "Add" methods is the `string` region of the worksheet that you would like to apply the conditional format to.

Many of the conditional formats that you can add to your worksheet have a `cellFormat` property that determines the way that the [`WorksheetCell`](/components/excel_library_using_worksheets.html) elements should look when the condition in that conditional format holds true. For example, you can use the properties attached to this `cellFormat` property such as `Fill` and `Font` to determine the background and font settings of your cells under a particular conditional format, respectively.

There are a few conditional formats that do not have a `cellFormat` property, as their visualization on the worksheet cell behaves differently. These conditional formats are the [`DataBarConditionalFormat`](/components/excel_library_using_worksheets.html), [`ColorScaleConditionalFormat`](/components/excel_library_using_worksheets.html), and [`IconSetConditionalFormat`](/components/excel_library_using_worksheets.html).

When loading a pre-existing [`Workbook`](/components/excel_library_using_worksheets.html) from Excel, the formats will be preserved when that [`Workbook`](/components/excel_library_using_worksheets.html) is loaded. The same is true for when you save the [`Workbook`](/components/excel_library_using_worksheets.html) out to an Excel file.
