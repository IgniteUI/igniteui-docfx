...
The map's `Infragistics.Controls.Charts.Series` property is used to support rendering an unlimited number of geographic series. This property is a collection of geographic series objects and any type of geographic series can be added to it. For example, [`IgxGeographicSymbolSeriesComponent`](/components/map_overview.html) can be added for plotting geographic locations such as cities and the [`IgxGeographicPolylineSeriesComponent`](/components/map_overview.html) for plotting connections (e.g. roads) between these geographic locations.
...
The [`IgxGeographicMap`](/components/map_overview.html) requires the following modules:
