...
While browsers natively provide search functionality, most of the time the Tree Grid virtualizes its columns and rows that are out of view. In these cases, the native search is unable to search the virtualized cells, since they are not part of the DOM. We have extended the Ignite UI for Angular Tree Grid with a **search API** that allows you to search through the **virtualized content** of the Tree Grid.
