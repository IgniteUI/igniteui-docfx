...
The combo also fires an event every time its selection changes - [selectionChange()]({environment:angularApiUrl}/classes/igxcombocomponent.html#selectionchange). The emitted event arguments, [IComboSelectionChangeEventArgs]({environment:angularApiUrl}/interfaces/icomboselectionchangeeventargs.html), contain information about the selection prior to the change, the current selection and the items that were added or removed. The event can also be cancelled, preventing the selection update with the new array of items.
...
<igx-combo [data]="cities" displayKey="name" valueKey="id"
           (selectionChange)="handleCityChange($event)">
</igx-combo>
```
...
By default, the combo control provides multiple selection. The example below demonstrates how to achieve single selection in the component by attaching a handler to the `selectionChange` event:

```html
<igx-combo [data]="lData" (selectionChange)="singleSelection($event)"></igx-combo>
```
