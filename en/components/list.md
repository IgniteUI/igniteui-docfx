...
At its core the Angular list module allows you to easily display a vertical list of items. The default styling of the items is done according to the single-line list specification as per the Material Design [**guidelines**](https://material.io/guidelines/components/lists.html).

To get started with the Angular list comonent, first you need to install Ignite UI for Angular by typing the following command:
