...
The Ignite UI for Angular data chart component allows for placement of horizontal or vertical lines at a single numeric value that you define through usage of the [`IgxValueOverlayComponent`](/components/datachart_series_value_overlay.html). This can help you to visualize data such as the mean or median of a particular series.
...
Unlike other series types that use a `DataSource` for data binding, the value overlay uses a [`valueMemberPath`](/components/datachart_series_value_overlay.html) property to bind a single numeric value. In addition, the value overlay requires you to define a single [`axis`](/components/datachart_series_value_overlay.html) to use. If you use an X-axis, the value overlay will be a vertical line, and if you use a Y-axis, it will be a horizontal line.

When using a numeric X or Y axis, the [`valueMemberPath`](/components/datachart_series_value_overlay.html) property should reflect the actual numeric value on the axis where you want the value overlay to be drawn. When using a category X or Y axis, the [`valueMemberPath`](/components/datachart_series_value_overlay.html) should reflect the index of the category at which you want the value overlay to appear.
...
[`IgxValueOverlayComponent`](/components/datachart_series_value_overlay.html) appearance properties are inherited from `Infragistics.Controls.Charts.Series` and so [`brush`](/components/datachart_series_value_overlay.html) and [`thickness`](/components/datachart_series_value_overlay.html) for example are available and work the same way they do with other types of series.
