...
You can access a [`WorksheetCell`](excel_library_using_cells.md) object or a [`WorksheetRegion`](excel_library_using_cells.md) object by calling the [`worksheet`](excel_library_using_cells.md) object’s `getCell` or `getRegion` methods, respectively. Both methods accept a string parameter that references a cell. Getting a reference to a cell is useful when applying formats or working with formulas and cell contents.
...
The Infragistics Excel Library supports the referencing of cells and regions by name through the `getCell` and `getRegion` methods of the [`worksheet`](excel_library_using_cells.md) object. You refer to the cell or region using the [`NamedReference`](excel_library_using_cells.md) instance that refers to that cell or region.
...
The Infragistics Excel Library allows you to add Microsoft Excel formulas to a cell or group of cells in a worksheet. You can do this using the [`WorksheetCell`](excel_library_using_cells.md) object’s [`applyFormula`](excel_library_using_cells.md) method or by instantiating a [`formula`](excel_library_using_cells.md) object and applying it to a cell. Regardless of the manner in which you apply a formula to a cell, you can access the [`formula`](excel_library_using_cells.md) object using the [`WorksheetCell`](excel_library_using_cells.md) object’s [`formula`](excel_library_using_cells.md) property. If you need the value, use the cell’s [`value`](excel_library_using_cells.md) property.
...
The Infragistics Excel Library allows you to customize the look and behavior of a cell. You can customize a cell by setting properties exposed by the `cellFormat` property of the [`WorksheetCell`](excel_library_using_cells.md), [`WorksheetRow`](excel_library_using_cells.md), [`WorksheetColumn`](excel_library_using_cells.md), or [`WorksheetMergedCellsRegion`](excel_library_using_cells.md) objects.
...
You can create all possible fill types using static properties and methods on the [`CellFill`](excel_library_using_cells.md) class. They are as follows:
...
-   `createSolidFill` - Returns a [`CellFillPattern`](excel_library_using_cells.md) instance which has a pattern style of `Solid` and a background color set to the `Color` or [`WorkbookColorInfo`](excel_library_using_cells.md) specified in the method.

-   `createPatternFill` - Returns a [`CellFillPattern`](excel_library_using_cells.md) instance which has the specified pattern style and the `Color` or [`WorkbookColorInfo`](excel_library_using_cells.md) values, specified for the background and pattern colors.

-   `createLinearGradientFill` - Returns a [`CellFillLinearGradient`](excel_library_using_cells.md) instance with the specified angle and gradient stops.

-   `createRectangularGradientFill` - Returns a [`CellFillRectangularGradient`](excel_library_using_cells.md) instance with the specified left, top, right, and bottom of the inner rectangle and gradient stops. If the inner rectangle values are not specified, the center of the cell is used as the inner rectangle.

The derived types, representing the various fills which can be created, are as follows:

-   [`CellFillPattern`](excel_library_using_cells.md) - A pattern that represents a cell fill of no color, a solid color, or a pattern fill for a cell. It has background color info and a pattern color info which correspond directly to the color sections in the Fill tab of the Format Cells dialog of Excel.

-   [`CellFillLinearGradient`](excel_library_using_cells.md) - Represents a linear gradient fill. It has an angle, which is degrees clockwise of the left to right linear gradient, and a gradients stops collection which describes two or more color transitions along the length of the gradient.

-   [`CellFillRectangularGradient`](excel_library_using_cells.md) - Represents a rectangular gradient fill. It has top, left, right, and bottom values, which describe, in relative coordinates, the inner rectangle from which the gradient starts and goes out to the cell edges. It also has a gradient stops collection which describes two or more color transitions along the path from the inner rectangle to the cell edges.

The following code snippet demonstrates how to create a solid fill in a [`WorksheetCell`](excel_library_using_cells.md):
...
Colors are defined by the [`WorkbookColorInfo`](excel_library_using_cells.md) class, which is a sealed immutable class. The class has a static `automatic` property, which returns the automatic color, and there are various constructors which allow you to create a [`WorkbookColorInfo`](excel_library_using_cells.md) instance with a color or a theme value and an optional tint.

The `getResolvedColor` method on [`WorkbookColorInfo`](excel_library_using_cells.md) allows you to determine what color will actually be seen by the user when they open the file in Excel.

If the [`WorkbookColorInfo`](excel_library_using_cells.md) represents a theme color, you must pass in a Workbook instance to the method so it can get the theme color’s RGB value from the workbook.
...
You can set a host of different formats on a [`WorksheetCell`](excel_library_using_cells.md) by using the [`cellFormat`](excel_library_using_cells.md) object returned by the `cellFormat` property of that cell. This [`cellFormat`](excel_library_using_cells.md) object enables you to style many different aspects of the cell such as borders, font, fill, alignments, and whether or not the cell should shrink to fit or be locked.

You can also access the built-in styles to Microsoft Excel 2007 using the `styles` collection of the [`Workbook`](excel_library_using_cells.md) object. The full list of styles in Excel can be found in the Cell Styles gallery of the Home tab of Microsoft Excel 2007.
...
With this feature, a `style` property has been added to the `CellFormat` object. This is a reference to a [`WorkbookStyle`](excel_library_using_cells.md) instance, representing the parent style of the format. For formats of a style, this property will always be null, because styles cannot have a parent style. For row, column, and cell formats, the `style` property always returns the `normalStyle` by default.
...
When you merge cells, each cell in the region will have the same value and cell format. The merged cells will also be associated with the same [`WorksheetMergedCellsRegion`](excel_library_using_cells.md) object, accessible from their `associatedMergedCellsRegion` property. The resultant [`WorksheetMergedCellsRegion`](excel_library_using_cells.md) object will also have the same value and cell format as the cells.
...
In order to create a merged cell region, you must add a range of cells to the [`Worksheet`](excel_library_using_cells.md) object’s `mergedCellsRegions` collection. This collection exposes an `Add` method that takes four integer parameters. The four parameters determine the index of the starting row and column (top-left most cell) and the index of the ending row and column (bottom-right most cell).
